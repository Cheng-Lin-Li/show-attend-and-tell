...
class BaseModel(object):
    ...
    def eval(self, sess, eval_gt_coco, eval_data, vocabulary):
        ...
        for k in tqdm(list(range(eval_data.num_batches)), desc='batch'):
            batch = list(set(eval_data.next_batch()))
...
#                 image_name = os.path.splitext(image_name)[0]
        ...
        scorer = COCOEvalCap(eval_gt_coco, eval_result_coco, eval_data)
        scorer.evaluate()
...
#                 image_name = os.path.splitext(image_name)[0]
    ...
    def load_cnn(self, session, data_path, ignore_missing=True):
        ...
        data_dict = np.load(join(os.getcwd(), data_path), encoding='latin1').item()
        count = 0
